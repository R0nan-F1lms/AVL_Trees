...
class AVLTree:
    ...
    def plot(self, root, rotation_type=None):
        ...
        # Insert other explanations or details here
        self.add_explanations(rotation_type)
    ...
    def get_internal_and_external_nodes(self, root):
        internal_nodes = []
        external_nodes = []

        # Perform a level-order traversal to classify internal and external nodes
        queue = [root]
        while queue:
            node = queue.pop(0)
            if node.left or node.right:
                internal_nodes.append(node.key)
            else:
                external_nodes.append(node.key)
    ...
    def add_explanations(self, rotation_type):
        # Add detailed text annotations to the plot for each action
        if self.step > 0:
            if rotation_type:
                explanation = f"{rotation_type} rotation required"
            else:
                explanation = f"Inserting node"

            plt.text(-3, 0.6, f"{explanation}", fontsize=12, weight='bold')

            # Get internal and external nodes
            internal_nodes, external_nodes = self.get_internal_and_external_nodes(self.root)

            # Show the internal and external nodes
            plt.text(-3, 0.4, f"Internal Nodes: {internal_nodes}", fontsize=10)
            plt.text(-3, 0.3, f"External Nodes: {external_nodes}", fontsize=10)

            # Showing the imbalanced node
            if self.imbalanced_node:
                plt.text(-3, 0.2, f"Imbalance at node {self.imbalanced_node.key}", fontsize=10, color="red")
